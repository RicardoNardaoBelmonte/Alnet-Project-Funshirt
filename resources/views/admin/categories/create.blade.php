<x-layouts::main-content :title="__('New Category')"
    ...
    subheading="Add a new category to the shop">
    ...
    <div class="p-6 max-w-xl">
        ...
        <div class="bg-white dark:bg-zinc-800 rounded-2xl border border-zinc-200 dark:border-zinc-700 p-8">
            <form action="{{ route('admin.categories.store') }}" method="POST" enctype="multipart/form-data" class="flex flex-col gap-6">
                @csrf
                @include('admin.categories.partials.form', ['category' => null])
                <div class="flex gap-3 pt-2">
                    <flux:button type="submit" variant="primary" class="flex-1">Create Category</flux:button>
                    <a href="{{ route('admin.categories.index') }}"
